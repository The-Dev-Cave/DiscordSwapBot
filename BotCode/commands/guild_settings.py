...
async def guild_settings(ctx: lightbulb.SlashContext):
    ...
    await conn.close()
    embed = hikari.Embed(title=f"{bot_name} Settings", description=f"The following are the current settings for {bot_name}.  Use `/set (setting) (option)` to change the settings")
    ...
    embed.add_field(f"Post Approval = {guild_settings_row.get('post_approval')}", "Do moderators need to approv`e posts before everyone can see and interact with them. \n</set post-approval:1066868724008755251> \n**Default: True**", inline=True)
    embed.add_field(f"Profile Enabled = {guild_settings_row.get('profile_enabled')}", "Is viewing the profile of another user allowed. Profiles are a name, pronouns, and ratings. \n</set profile-enabled:1066868724008755251> \n**Default: True**", inline=True)
    embed.add_field(f"Profile Required = {guild_settings_row.get('profile_required')}", "Do people need a profile to to make and interact with posts. \n</set profile-required:1066868724008755251> \n**Default: False**", inline=True)
    embed.add_field(f"Ratings Enabled = {guild_settings_row.get('ratings_enabled')}", "Can people see other's ratings and can they rate each other after making a transaction with someone. Ratings are global \n</set ratings-enabled:1066868724008755251>\n**Default: True**", inline=True)
    embed.add_field(f"Listing Delay = {guild_settings_row.get('listing_delay')} Minutes", "How many minutes between making posts. \n</set listing-delay:1066868724008755251>\n**Default: 0**", inline=True)
    embed.add_field(f"Post Expiry = {guild_settings_row.get('expiry_time')} Days", "How many days it takes a post to expire. \n</set expiry:1066868724008755251> \n**Default: 7**", inline=True)
    embed.add_field(f"Post Renewal = {guild_settings_row.get('renewal_count')} Days", "How many times someone can repost without needing to remake it. \n</set renew:1066868724008755251> \n**Default: 3**", inline=True)
    embed.add_field(f"Mod Role", f"<@&{guild_settings_row.get('mod_role_id')}>\n</set mod-role:1066868724008755251>", inline=True)
    ...
    embed.add_field(f"Postings Channels", f"Sell - {sell_chnl.mention}\n</set sell-posts-chnl:1066868724008755251>\n\nBuy - {buy_chnl.mention}\n</set buy-posts-chnl:1066868724008755251>", inline=True)
    embed.add_field(f"Post Approval Channel", f"<#{guild_settings_row.get('approval_channel_id')}>\n\n</set post-approval-chnl:1066868724008755251>", inline=True)
    embed.add_field(f"Logs", f"Public = <#{guild_settings_row.get('logs_channel_id')}>\n</set public-logs-chnl:1066868724008755251>\n\nPublic = <#{guild_settings_row.get('mod_log_channel_id')}>\n</set mod-logs-chnl:1066868724008755251>", inline=True)
