...
@lightbulb.command("expiry", "days it takes for a post to expire", auto_defer=True, ephemeral=True)
...
@lightbulb.command("post-approval", "Do posts need to be approved by mods before everyone can see it", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("bool", "True or False", type=bool, required=True)
@lightbulb.command("profile-enabled",
                   "Is viewing the profile of another user allowed. Profiles are a name, pronouns, and ratings", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("bool", "True or False", type=bool, required=True)
@lightbulb.command("profile-required", "Do people need a profile to to make and interact with posts", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
async def profile_required(ctx: lightbulb.SlashContext):
    ...
    conn = await get_database_connection()
    if option:
        await conn.execute("UPDATE guilds set profile_required=$1, profile_enabled=TRUE where guild_id=$2", option,
                           guild_id)
        await send_mod_log(guild_id=guild_id,
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("bool", "True or False", type=bool, required=True)
@lightbulb.command("ratings-enabled",
                   "Can people see other's ratings and rate each other after making a transaction with someone", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.command("renew", "How many times a user can renew a post without needing to remake", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.command("buy-posts-chnl", "The text channel that buy listings are posted", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("channel", "the channel", type=hikari.TextableGuildChannel, required=True)
@lightbulb.command("sell-posts-chnl", "The text channel that sell listings are posted", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("channel", "the channel", type=hikari.TextableGuildChannel, required=True)
@lightbulb.command("post-approval-chnl", "The text channel that approval of listings is done", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("channel", "the channel", type=hikari.TextableGuildChannel, required=True)
@lightbulb.command("public-logs-chnl", "The text channel for public logs to go", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
async def approval_channel(ctx: lightbulb.SlashContext):
    guild_id = ctx.guild_id
    channel = ctx.options.channel
...
@lightbulb.app_command_permissions(perms=hikari.Permissions.ADMINISTRATOR, dm_enabled=False)
@lightbulb.option("channel", "the channel", type=hikari.TextableGuildChannel, required=True)
@lightbulb.command("mod-logs-chnl", "The text channel for mod logs to go", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
async def approval_channel(ctx: lightbulb.SlashContext):
    guild_id = ctx.guild_id
    channel = ctx.options.channel
...
@lightbulb.command("mod-role", "The role that can interact with SwapBot for moderator actions", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
async def approval_channel(ctx: lightbulb.SlashContext):
    guild_id = ctx.guild_id
...
@lightbulb.command("user-bridge-cat", "Category where channels between users are made", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
async def approval_channel(ctx: lightbulb.SlashContext):
    guild_id = ctx.guild_id
...
@lightbulb.command("listing-delay", "Minutes between making posts. 0 for no delay, 1440 for 1 day", auto_defer=True, ephemeral=True)
@lightbulb.implements(lightbulb.SlashSubCommand)
...
async def guild_settings(ctx: lightbulb.SlashContext):
    ...
    guild_settings_row = await conn.fetchrow(
        ...
        ctx.guild_id)
    await conn.close()
    embed = hikari.Embed(title=f"{bot_name} Settings",
    ...
    sell_chnl = await ctx.bot.rest.fetch_channel(guild_settings_row.get('sell_channel_id'))
    buy_chnl = await ctx.bot.rest.fetch_channel(guild_settings_row.get('buy_channel_id'))

    embed.add_field(f"Post Approval = {guild_settings_row.get('post_approval')}",
                    "Do moderators need to approv`e posts before everyone can see and interact with them. \n</set post-approval:1066868724008755251> \n**Default: True**",
                    inline=True)
    embed.add_field(f"Profile Enabled = {guild_settings_row.get('profile_enabled')}",
                    ...
                    inline=True)
    embed.add_field(f"Profile Required = {guild_settings_row.get('profile_required')}",
                    "Do people need a profile to to make and interact with posts. \n</set profile-required:1066868724008755251> \n**Default: False**",
                    inline=True)
    embed.add_field(f"Ratings Enabled = {guild_settings_row.get('ratings_enabled')}",
                    ...
                    inline=True)
    embed.add_field(f"Listing Delay = {guild_settings_row.get('listing_delay')} Minutes",
                    "How many minutes between making posts. \n</set listing-delay:1066868724008755251>\n**Default: 0**",
                    inline=True)
    embed.add_field(f"Post Expiry = {guild_settings_row.get('expiry_time')} Days",
                    "How many days it takes a post to expire. \n</set expiry:1066868724008755251> \n**Default: 7**",
                    inline=True)
    embed.add_field(f"Post Renewal = {guild_settings_row.get('renewal_count')} Days",
                    "How many times someone can repost without needing to remake it. \n</set renew:1066868724008755251> \n**Default: 3**",
                    inline=True)
    embed.add_field(f"Mod Role", f"<@&{guild_settings_row.get('mod_role_id')}>\n</set mod-role:1066868724008755251>",
                    inline=True)
    embed.add_field(f"Channels", "The channels set for this guild and how to change them", inline=False)
    embed.add_field(f"User Bridge Category = {cat.name}", "</set user-bridge-cat:1066868724008755251>", inline=True)
    embed.add_field(f"Postings Channels",
